...
- `CMakeLists.txt` — fetches a pinned GoogleTest, compiles the C sources
  into the test binary (adjust the glob to the project layout), links, and registers tests. Normally unedited; the one
...
- `build_fuzz.sh` — build in fuzzing mode (Clang, coverage-guided campaign).
- `docs/` — the vendored official FuzzTest reference docs (Apache-2.0).
